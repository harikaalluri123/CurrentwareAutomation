...
export class OperatorsPage {
    ...
    async removeOperator(username: string, transferSubscription?: boolean): Promise<void> {
        ...
        await Promise.all([
            this.page.waitForResponse(res =>
                res.url().includes('deleteOperator') &&
    ...
    async isOperatorPresent(operatorName: string): Promise<boolean> {
        const locator = this.page.getByText(operatorName, { exact: true });
        if (await locator.count() > 0) {
            return true;
        }
        return await this.findOperatorInPagination(operatorName);
    }
